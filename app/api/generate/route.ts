...
4. **深度闭环**：业务闭环 (loop) 必须极度详实、逻辑自洽且具备实操性。严禁出现“开发产品”、“寻找客户”这种空洞的废话。每个步骤必须是 20-40 字的完整描述，详细解释具体的执行策略（如：通过具体的渠道获客、采用具体的模式变现等）。注意：因为 UI 已经自带序号，JSON 里的文字严禁包含任何序号前缀。
...

{
  ...
  "loop": {
    "desc": "一句话描述业务闭环的核心逻辑",
    "steps": [
      "如何获取流量/初始客户 (直接描述动作，不要包含“第一步”等序号)",
      "如何交付产品/核心价值 (直接描述动作，不要包含“第二步”等序号)",
      "如何留存用户/建立壁垒 (直接描述动作，不要包含“第三步”等序号)",
      "如何赚钱/实现盈利 (直接描述动作，不要包含“第四步”等序号)",
      "如何规模化/持续增长 (直接描述动作，不要包含“第五步”等序号)"
    ],
    "type": "Cycle"
  },
  "saas_hook": {
